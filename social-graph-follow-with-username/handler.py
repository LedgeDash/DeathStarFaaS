...
def handle(req):
    """given usename and followee_name, look up user_id and followee_id, add 
    followee_id to user_id's followees list and user_id to followee_id's
    followers list
    Args:
    ...
    r = requests.get("http://gateway.openfaas:8080/function/social-graph-follow",
            data=req_str)

    if r.status_code != 200:
        message = clean_error_msg(r.text)
        sys.exit(json.dumps({"status":"FollowError",
                             "message": "social-graph-follow failed. {}".format(message)}))

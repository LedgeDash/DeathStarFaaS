...
def handle(req):
    ...
    if hlen_reply == int(os.getenv('NUM_COMPONENTS')):
        ...
        if ret['http_status_code']!= 200:
            sys.exit(dumps({"status":"ComposePostServiceUploadUrlsError", "req_id": req_id_str,
                              "errors": [ret]}))
        else:
